...
public class OrderDao {
	...
	public void addProduct(Order order) throws SQLException {
        ...
        // 3.执行update()方法插入数据
		runner.update(DataSourceUtils.getConnection(), sql, order.getId(),order.getMoney(), 
				order.getReceiverAddress(), order.getReceiverName(), order.getReceiverPhone(), order.getUser().getId());
	}
	/**
	 * 查找用户所属订单
	 */
		...
		return runner.query(sql, new ResultSetHandler<List<Order>>() {
			public List<Order> handle(ResultSet rs) throws SQLException {
				...
				while (rs.next()) {
					...
					order.setSendstate(rs.getInt("sendstate"));
					order.setReceivestate(rs.getInt("receivestate"));
					order.setEvaluatestate(rs.getInt("evaluatestate"));
	...
	public void updateOrderState(String id) throws SQLException {
		...
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		System.out.println("成功支付订单数："+runner.update(sql, id));
	}
	/**
	 *  根据订单号修改订单发货状态
	 * @param id
	 * @throws SQLException
	 */
	public void updateSendState(String id) throws SQLException {
		String sql = "update orders set sendstate=1 where id=?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		System.out.println("修改订单发货状态条数："+runner.update(sql, id));
	}
	/**
	 *  根据订单号修改订单收货状态
	 * @param id
	 * @throws SQLException
	 */
	public void updateReceiveState(String id) throws SQLException {
		String sql = "update orders set receivestate=1 where id=?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		System.out.println("修改订单收货状态条数："+runner.update(sql, id));
	}
	/**
	 *  根据订单号修改订单评价状态
	 * @param id
	 * @throws SQLException
	 */
	public void updateEvaluateState(String id) throws SQLException {
		String sql = "update orders set evaluatestate=1 where id=?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		System.out.println("修改订单评价状态条数："+runner.update(sql, id));
	}
	/**
	 *  多条件查询
	 * @param id
