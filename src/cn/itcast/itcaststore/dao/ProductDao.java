...
public class ProductDao {
	...
	// 根据id查找商品后增加商品评价
	public void increaseevaluate(String id) throws SQLException {
		String sql = "update products set evaluate=(evaluate+1) where id=?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		System.out.println("商品增加评价"+runner.update(sql, id));
	}
	// 根据id查找商品后降低商品评价
	public void reduceevaluate(String id) throws SQLException {
		String sql = "update products set evaluate=(evaluate-1) where id=?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		System.out.println("商品降低评价"+runner.update(sql, id));
	}
	// 生成订单时，将商品数量减少
	...
	// 查看商品当月销量
		public List<Object[]> salesnumber(String year, String month,String id)
				throws SQLException {
			String sql = "SELECT SUM(orderitem.buynum) totalsalnum "
					+ "FROM orders,products,orderItem "
					+ "WHERE orders.id=orderItem.order_id AND products.id=orderItem.product_id "
					+ "AND orders.paystate=1 and year(ordertime)=? and month(ordertime)=? and products.id=?"
					+ "GROUP BY products.name ORDER BY totalsalnum DESC";
			QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
			return runner.query(sql, new ArrayListHandler(), year, month, id);
		}
		// 查看一类商品当月销量
		public List<Object[]> salescategorynumber(String year, String month,String category)
				throws SQLException {
			String sql = "SELECT products.id,SUM(orderitem.buynum) totalsalnum "
					+ "FROM orders,products,orderItem "
					+ "WHERE orders.id=orderItem.order_id AND products.id=orderItem.product_id "
					+ "AND orders.paystate=1 and year(ordertime)=? and month(ordertime)=? and products.category=?"
					+ "GROUP BY products.name ORDER BY products.id DESC";
			QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
